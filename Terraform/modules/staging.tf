...
# =============================================================
# Creates a separate ConfigMap in the 'staging' namespace so
# the staging Helm release can connect to the same RDS/Redis
# but in an isolated namespace. Shares the same backing infra
# as the dev environment (same cluster, same RDS, same Redis).
# =============================================================
...
resource "kubernetes_config_map_v1" "staging_config" {
  metadata {
  ...
  data = {
    ...
    # Secret Names (same secrets as production - shared infra)
